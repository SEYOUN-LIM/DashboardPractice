...
namespace dashboard.API
{
    public class DataSeed
    {
        ...
        {
            if (!EnumerableExtensions.Any(_ctx.Customers))
            {
                SeedCustomers(nCustomers);
                _ctx.SaveChanges();
            }

            if (!EnumerableExtensions.Any(_ctx.Orders))
            {
                SeedOrders(nOrders);
                _ctx.SaveChanges();
            }

            if (!EnumerableExtensions.Any(_ctx.Servers))
            {
                SeedServers();
                _ctx.SaveChanges();
            }
        ...
        private List<Order> BuildOrderList(int nOrders)
        {
            ...
            for (var i = 1; i <= nOrders; i++)
            {
                ...
                {
                    Id = i,
                    Customer = customers.FirstOrDefault(c => c.Id == randCustomerId),
