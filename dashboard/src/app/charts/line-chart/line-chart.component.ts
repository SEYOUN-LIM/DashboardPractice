  ...
  ngOnInit() {
    this._salesDataService.getOrders(1, 100).subscribe(res => {
      ...
      this._salesDataService.getOrdersByCustomer(3).subscribe(cus => {
        this.topCustomers = cus.map(x => x['state']);
        ...
        this.lineChartData = [
          { 'data': r[0].orders.map(x => x.orderTotal), 'label': r[0]['customer']},
          { 'data': r[1].orders.map(x => x.orderTotal), 'label': r[1]['customer']},
          { 'data': r[2].orders.map(x => x.orderTotal), 'label': r[2]['customer']}
        ];
    ...
    customers.reduce((x, y, i) => {
      ...
      dataSets[i] = {
        ...
        u.reduce((r, e, j) => {
          const obj = {};
          obj['date'] = e;
          obj['orderTotal'] = this.getCustomerDateTotal(e, y); // sum total orders for this customer on this day
